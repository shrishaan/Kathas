...
const LikeCount = ({ props }) =>  {
  ...
  const handleLike = async () => {
    try {
      if(!user.isLoggedIn){
        return showToast('error', 'You must be logged in to like the blog.');
      }
  ...
  return (
    <button onClick={handleLike} type="button" className="flex justify-between items-center gap-1 text-gray-500" >
      {!hasLiked ?   
      <FaRegHeart />
      :
       <FaHeart fill='red'/>
